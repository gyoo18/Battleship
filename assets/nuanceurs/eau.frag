...
void main(){
    ...
    vec3 norm_l = -normalize(cross(dFdx(vec3(uv_O.x,vague_hauteur_l,uv_O.y)),dFdy(vec3(uv_O.x,vague_hauteur_l,uv_O.y))));
    ...
    vec3 dirVagues = vec3(cos(AV),0,sin(AV));
    float derriere = 10.0*max(min(dot(norm_l,dirVagues),1.0),0.0);
    ...
    float ecume_traces = 1.0*max(sin(50.0*(u+d))*sin(50.0*(u+d))*sin(50.0*(u+d)),0.0)*sqrt(derriere)*sqrt(vague_hauteur_l)*sqrt(1.0-vague_hauteur_l);
    float b = 0;
    for (int i = 1; i <= 4; i++){
        b += (2.0/(exp(1.5*i)*i))*bruitp(0.05*posMonde.x - cos(AV)*(0.1/i)*temps,0.05*posMonde.z - sin(AV)*(0.1/i)*temps,(0.1/i)*temps,1.0/(i*i*i));
    }
    float ecume = max(ecume_traces,cretes)*b;
    ...
    Fragment = vec4(mix(Coul.rgb,vec3(0.8,0.8,0.8),ecume)*diff + spec, Coul.a);
}